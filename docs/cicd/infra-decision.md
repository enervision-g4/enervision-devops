...
collecteur, bus Kafka, consommateurs, base TimescaleDB) s'exécute sur le
**serveur on-premise de l'école**, en conteneurs orchestrés par Docker
Compose (`compose/`).

> La façon d'orchestrer ces conteneurs sur ce serveur est une décision
> *distincte* de celle documentée ici, et elle est encore ouverte : voir
> la section « Ce que ce document ne tranche pas ». Le raisonnement
> ci-dessous porte sur le **lieu** d'exécution et sur le rôle du cloud,
> pas sur l'orchestrateur.
...
- **Localisation.** `location = "francecentral"` dans
  `infra/terraform/main.tf` : les sauvegardes ne quittent pas le
...
pouvoir grandir sans réécriture. Deux garanties concrètes, vérifiables
dans le code de ce dépôt :

- rien n'est câblé sur le serveur de l'école. Les fichiers Compose, les
  variables d'environnement et les modules Terraform ne contiennent
  aucune adresse ni aucun nom propre à cette machine ;
- la cible des sauvegardes se change avec une seule variable,
  `RESTIC_REPOSITORY` — Azure Blob, S3, SFTP ou disque local. Sortir
...
1. **Rétention par `restic forget`**, pas par une règle de cycle de vie
   ...
   tier — voir `infra/terraform/main.tf`.
...
- pas de haute disponibilité — une seule machine, une seule
  alimentation, un seul disque. Si elle tombe, tout tombe ;
...
- **Tout est décrit en code** : `infra/terraform/` recrée les ressources
  Azure à l'identique, et le déploiement passe par des workflows
  versionnés, pas par des commandes tapées à la main. La reconstruction
  est un fichier, pas un souvenir.
...
**Un fournisseur cloud européen plutôt qu'Azure**, pour les
sauvegardes. Recevable, et c'est même l'échappatoire prévue si l'exigence
de souveraineté se durcit (voir plus bas). Écarté aujourd'hui pour une
raison de cohérence : la zone externe déjà prévue en EC01 — archive long
terme et module ML — était planifiée sur Azure. Ouvrir un troisième
fournisseur pour quelques gigaoctets de sauvegardes aurait ajouté un
contrat, une console et un jeu d'identifiants de plus, sans rien
résoudre.
...
**Comment les conteneurs sont orchestrés sur ce serveur.** Aujourd'hui
c'est Docker Compose, service par service. Le passage à un orchestrateur
(k3s est le candidat évalué) apporterait l'isolation structurelle entre
dev et prod, le retour arrière natif et des sondes de vivacité gérées —
au prix d'un vocabulaire supplémentaire à maîtriser. Cette décision est
indépendante de celle-ci : quel que soit l'orchestrateur, les données
restent sur le serveur de l'école et Azure ne reçoit que des sauvegardes
chiffrées. Elle fera l'objet de sa propre argumentation.
...
`infra/terraform/main.tf` est spécifique, et il
n'approvisionne qu'un compte de stockage.

## Voir aussi

- `infra/terraform/README.md` — ce que le code Terraform crée réellement
